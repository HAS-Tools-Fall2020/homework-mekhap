...
# Step 11: Generate plots to visualize regression model and datasets
...
# PLOT 1: Model Period
ax.plot(train['flow_log'], 'orange', label='training period', linewidth=6)
ax.plot(test['flow_log'], 'greenyellow', label='testing period', linewidth=6)
ax.plot(data['flow_log'], 'b', label='observed flow', linewidth=1)
ax.set(title="Model Period (2017-2020)",
          xlabel="Date", ylabel="Weekly Avg Natual Log Flow [cfs]",
          xlim=['2017-01-07', '2020-12-05'])
ax.legend(fontsize=8)
plt.setp(ax.get_xticklabels(), rotation=45)
plt.show()
# Save Figure
fig.savefig("TrainTestPeriods", bbox_inches='tight')
...
ax.plot(data['flow_log_nm'], 'navy', label='observed flow')
ax.plot(data['flow_tm1_nm'], 'cornflowerblue', label='lagged flow')
ax.plot(data['precip_nm'], 'forestgreen', label='precip')
ax.plot(data['airtemp_nm'], 'orange', label='temperature')
ax.set(title="Normalized Predictive Variables", xlabel="Date",
          xlim=['2017-01-07', '2018-12-29'])
ax.legend(fontsize=8)
plt.setp(ax.get_xticklabels(), rotation=45)
plt.show()
# Save Figure
fig.savefig("InputVariable", bbox_inches='tight')
...
ax.scatter(np.sort(test['flow_log']), np.sort(q_pred_test),
              color='m', edgecolor='purple', label='Model Predictions')
ax.plot([0, 9], [0, 9], color='blue', alpha=0.5)
ax.set(title="Model Performance - Testing Period", xlabel="Observed Natural Log Flow",
          ylabel="Simulated Natural Log Flow", xlim=[3, 9], ylim=[3, 9])
ax.legend(fontsize=8)
plt.setp(ax.get_xticklabels(), rotation=45)
plt.show()
# Save Figure
fig.savefig("ModelPerformance", bbox_inches='tight')

# Step 12: Forecast predictions
