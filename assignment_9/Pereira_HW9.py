...
def regmodel_2wk_predict(flow1, precip1, temp1):
    ...
    max temperature, all weekly average variables. The model should be named
    ...
    Inputs = (ints or floats) average flow for three consecutive weeks:
           ...
           precip1 = avgerage precipitation from week prior
           temp1 = avgerage max temperature from week prior

    Output = (array) array containing two values which represent 1 and 2 week
    forecast based on the regression model
           reg_model_predictions[0] = 1 week prediciton of avgerage flow
           reg_model_predictions[1] = 2 week prediciton of avgerage flow
    """
    # LC - wow nice logic in here!
    reg_model_predictions = np.zeros(2)
    for i in range(0, 2):
        reg_model_predictions[i] = model.intercept_ \
                                   + model.coef_[0] * flow1 \
                                   + model.coef_[1] * precip1 \
                                   + model.coef_[2] * temp1
        print('AR model ', i+1, ' week forecast is ',
              reg_model_predictions[i].round(2), ' cfs'
              )
        flow1 = reg_model_predictions[i]
...
# %%
# Step 4: Read in daymet precip data
...
# %%
# Step 5: Read in daymet max temperature data
...
# %%
# Step 10: Generate plots to visualize regression model and datasets
...
# %%
# Step 12: Make predictions for forecast entries outside of model

# Timeseries of observed flow values shows past 7 weeks have seen increasing trend
fig, ax = plt.subplots()
ax.plot(data['flow'].iloc[-7:],'b', label='observed flow', linewidth = 1, marker='o')
ax.set(title="Observed Flow for Forecast Period", xlabel="Date", 
       ylabel="Weekly Avg Flow [cfs]", ylim=[0,100])
ax.legend()
plt.show()

# Time series shows we have seen about a 4cfs per week increase over past 5 weeks
# Based predictions recent trend
myprediction_1wk = data['flow'].iloc[-1:].values[0].round(0) + 4
myprediction_2wk = myprediction_1wk + 4
...
# %%
# Step 13: Make predictions for 16 week forecast
